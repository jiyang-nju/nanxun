...
import { clampPoint, MAP_BOUNDS } from '../lib/game';
import type { Point } from '../lib/game';

type View = typeof MAP_BOUNDS;
interface Props {
  guess: Point | null;
  answer?: Point;
...
function bounded(view: View): View {
  return {
    ...view,
    x: Math.max(MAP_BOUNDS.x, Math.min(MAP_BOUNDS.x + MAP_BOUNDS.width - view.width, view.x)),
    y: Math.max(MAP_BOUNDS.y, Math.min(MAP_BOUNDS.y + MAP_BOUNDS.height - view.height, view.y)),
  };
}
export default function CampusMap({ guess, answer, onGuess }: Props) {
  const svg = useRef<SVGSVGElement>(null);
  const [view, setView] = useState<View>(MAP_BOUNDS);
  ...
  const scale = MAP_BOUNDS.width / view.width;
  ...
  function zoom(factor: number) {
    ...
    const width = MAP_BOUNDS.width / newScale,
      height = MAP_BOUNDS.height / newScale;
    const center = guess ?? { x: 810, y: 675 };
    setView(bounded({ x: center.x - width / 2, y: center.y - height / 2, width, height }));
  }
  return (
    <div className={`map-shell ${answer ? 'map-revealed' : ''}`}>
      <div className="map-caption">
        <span className="map-dot" /> 鼓楼北园 <span>示意地图</span>
      </div>
      ...
      <svg
        ...
        onKeyDown={(e) => {
          ...
          if (moves[e.key]) {
            e.preventDefault();
            const next = clampPoint(moves[e.key]);
            onGuess(next);
            setView((v) => bounded({ ...v, x: next.x - v.width / 2, y: next.y - v.height / 2 }));
          }
        }}
        onPointerDown={(e) => {
        ...
        onPointerUp={(e) => {
          if (drag.current && !drag.current.moved && !answer)
            onGuess(clampPoint(position(e.clientX, e.clientY)));
          drag.current = null;
          setDragging(false);
        }}
        onPointerCancel={() => {
      ...
      <div className="map-controls">
        ...
        <button onClick={() => setView(MAP_BOUNDS)} aria-label="重置地图视野">
          <LocateFixed size={18} />
        </button>
      </div>
